...
class MemberTest < ActiveSupport::TestCase
  ...
  test "can_have_account? returns true for owner, parent, teen" do
    assert members(:owner_one).can_have_account?
  ...
  test "display_status returns correct status" do
    assert_equal 'owner', members(:owner_one).display_status
  ...
  test "owner? returns true for owner role" do
    assert members(:owner_one).owner?
    assert_not members(:parent_one).owner?
  end

  test "parent_or_above? returns true for owner and parent" do
    assert members(:owner_one).parent_or_above?
  ...
  test "display_role returns human-readable role" do
    assert_equal 'Owner', members(:owner_one).display_role
  ...
  test "scopes return correct members" do
    ...
    assert_includes family.members.owners, members(:owner_one)
    assert_includes family.members.invitable, members(:teen_one)
    assert_not_includes family.members.invitable, members(:owner_one)
    assert_includes family.members.with_accounts, members(:owner_one)
  end
end
